...
//ドロップした花に関するスクリプト　大半の処理がdropwoodと同じなのでそちらを参照
public class DropFlower : MonoBehaviour
{
    ...
    public GameObject WoodCollectPos;
    ...
    public void Track()
    {
        if (npcProcess == false)
        {
            ...
            Vector3 pos = WoodCollectPos.transform.position;
        ...
        else if (npcProcess == true)
        {
            ...
            CanTrack = false;
            Grabed = false;
            Invoke("PlusFlower", 0.6f);//0.6秒後に木をプラスする
        }
    ...
    void PlusFlower()
    {
        ...
        Carigge.GetComponent<CariggeManager>().GetFlower();
        if (npcProcess == true)
        {
            npcObject.GetComponent<Nav>().HandHaving = false;
